...
const createSwaggerMethods = options => methods =>
  methods.reduce((swaggerFile, info) => {
    ...
    let parameters = ``;
    parameters += createSwaggerQueries(options)(info.model.queries, info.key);
    parameters += createSwaggerParameters(options)(info.route);
    if (parameters !== '') {
...
const createSwaggerPaths = options => models => {
  ...
  swaggerFile += Object.keys(sortedMethods)
    .reduce((accumulator, route) => {
      const swaggerRoute = route
        .replace(/:\b\w+\b/g, '{$&}')
        .replace(/:/g, '');
      accumulator += `  ${swaggerRoute}:\r\n`;
